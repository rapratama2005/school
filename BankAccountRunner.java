...
public class BankAccountRunner //remember to change this
{
   ...
   //main function
    public static void main(String[] args) throws FileNotFoundException
    {
      ...
      boolean student = (input.nextLine().substring(0,1).toLowerCase().equals("y"));
      print("\n");
      if (student){//output
         StudentBankAccount account = new StudentBankAccount(name, 100);//student
         for (int i = 0; i < 3; i++){
            int val = randomInt(20, 50);
            if (randomInt(0, 1) == 0){//deposit or withdraw
               account.deposit(val);
               System.out.printf("\n%s deposited $%d\n", name, val);
            } else {
         ...
         BankAccount account = new BankAccount(name, 100);//regular
         for (int i = 0; i < 3; i++){
            int val = randomInt(20, 50);
            if (randomInt(0, 1) == 0){
               account.deposit(val);
